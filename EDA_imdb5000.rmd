...
```{r echo=FALSE}
# Substitui NA pelo valor médio (arredondado) para a variável facenumber_in_poster
...
IMDB_data <- subset(IMDB, select = -c(country, num_critic_for_reviews, num_user_for_reviews, language, color, genres, facenumber_in_poster, plot_keywords, movie_imdb_link, aspect_ratio, actor_1_name, actor_1_facebook_likes, actor_2_name, actor_2_facebook_likes, actor_3_name, actor_3_facebook_likes))
```
...
IMDB_data %>%
  ...
  top_n(20, imdb_score) %>%
  ggplot(aes(x=num_voted_users, y=imdb_score, color=content_rating, size=profit/1000000)) +
  geom_point() +
  geom_text_repel(aes(label=movie_title))
```
...
IMDB_data %>%
  top_n(20, budget) %>%
  ggplot(aes(x = imdb_score, y = profit/1000000, size = budget, color = content_rating)) + 
  geom_point() + 
...
gp1 <- ggplot(IMDB_data, aes(num_voted_users, profit/10^6, color=content_rating)) + 
...
gp2 <- IMDB_data %>%
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
  scale_colour_hue(l = 70, c = 150) +
  labs(title = "Quantidade de votos vs Lucro - Top 20",
       x = "Quantidade de votos",
       y = "Lucro (milhões de US$)",
       colour = "Classificação de Conteúdo",
       size = "Orçamento")
...
gp3 <- ggplot(IMDB_data, aes(num_voted_users, imdb_score, color=content_rating)) + 
  geom_point(alpha=0.5) +
  geom_hline(aes(yintercept = 6.9), linetype = 2) +
  theme_dark() +
  theme(plot.title = element_text(hjust = 0.5),
...
gp4 <- IMDB_data %>%
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
  scale_colour_hue(l = 70, c = 150) +
  labs(title = "Quantidade de votos vs IMDB Score - Top 20",
       x = "Quantidade de votos",
       y = "IMDB Score",
       colour = "Classificação de Conteúdo",
       size = "Orçamento")
...
gp5 <- ggplot(IMDB_data, aes(num_voted_users, budget/10^6, color=content_rating)) + 
  geom_point(alpha=0.5) +
...
gp6 <- IMDB_data %>%
  ...
  theme(plot.title = element_text(hjust = 0.5),
        plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
  scale_colour_hue(l = 70, c = 150) +
  labs(title = "Lucro vs IMDB Score - Top 20 (Maiores Orçamentos)",
       x = "IMDB Score",
       y = "Lucro",
       colour = "Classificação de Conteúdo",
       size = "Orçamento")
